...
/* void Registration::Refine() {
    if (m_preSliderHighlightPoints->GetNumberOfPoints() > 0) {
        auto choice =
            QMessageBox::question(this, "Warning",
                                  "If meshes are not aligned, this process "
                                  "will produce FAULTY DATA!"
...
/* void Registration::Morph() {
    vtkNew<vtkMultiBlockDataSet> someEmptyBlock;
    std::vector<int>* someEmptyList = nullptr;
    int nmFixed = m_templateHighlightPoints->GetNumberOfPoints();
    int nmSurfSliders = m_templateSliders->GetNumberOfPoints();

    Eigen::MatrixXd totalTempCoords(nmFixed + nmSurfSliders, 3);
    Eigen::MatrixXd totalCoords(nmFixed + nmSurfSliders, 3);

    int surfaceStartIds = nmFixed;

    for (int i = 0; i < nmFixed; i++) {
        totalTempCoords.operator()(i, 0) =
            m_templateHighlightPoints->GetPoint(i)[0];
        totalTempCoords.operator()(i, 1) =
            m_templateHighlightPoints->GetPoint(i)[1];
        totalTempCoords.operator()(i, 2) =
            m_templateHighlightPoints->GetPoint(i)[2];
    }
    for (int i = 0; i < nmSurfSliders; i++) {
        totalTempCoords.operator()(i + surfaceStartIds, 0) =
            m_templateSliders->GetPoint(i)[0];
        totalTempCoords.operator()(i + surfaceStartIds, 1) =
            m_templateSliders->GetPoint(i)[1];
        totalTempCoords.operator()(i + surfaceStartIds, 2) =
            m_templateSliders->GetPoint(i)[2];
    }

    for (int i = 0; i < nmFixed; i++) {
        totalCoords.operator()(i, 0) = m_targetHighlightPoints->GetPoint(i)[0];
        totalCoords.operator()(i, 1) = m_targetHighlightPoints->GetPoint(i)[1];
        totalCoords.operator()(i, 2) = m_targetHighlightPoints->GetPoint(i)[2];
    }
    for (int i = 0; i < nmSurfSliders; i++) {
        totalCoords.operator()(i + surfaceStartIds, 0) =
            m_preSliderHighlightPoints->GetPoint(i)[0];
        totalCoords.operator()(i + surfaceStartIds, 1) =
            m_preSliderHighlightPoints->GetPoint(i)[1];
        totalCoords.operator()(i + surfaceStartIds, 2) =
            m_preSliderHighlightPoints->GetPoint(i)[2];
    }

    qRegisterMetaType<Eigen::MatrixXd>("Eigen::MatrixXd");
    delete m_slidingThread;
    m_slidingThread = new SlidingThread(
        m_meshData, nmFixed, 0, 1, someEmptyList, nmSurfSliders, 0, 0, 1,
        someEmptyBlock, someEmptyBlock, totalTempCoords, totalCoords);
    m_slidingThread->setParent(this);

    connect(m_slidingThread, &SlidingThread::CoordinateChanged, this,
            &Registration::OnCoordinateChanged);
    connect(m_slidingThread, &SlidingThread::CoordinateNotChanged, this,
            &Registration::OnCoordinateNotChanged);
