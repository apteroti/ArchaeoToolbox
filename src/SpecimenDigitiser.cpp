...
SpecimenDigitiser::SpecimenDigitiser(vtkPolyData* data, MainWindow* parent)
    ...
    m_surfaceMaskPoly = vtkSmartPointer<vtkPolyData>::New();
    ...
    surfaceToolbar->addWidget(surfaceAddButton);
    ...
    surfaceToolbar->addWidget(surfaceLockButton);
    surfaceToolbar->addSeparator();
    ...
    surfaceToolbar->addWidget(surfaceIronButton);
    surfaceToolbar->addSeparator();
    ...
    surfaceToolbar->addWidget(surfaceSelectLabel);
    surfaceSelectComboBox = new QComboBox();
    surfaceToolbar->addWidget(surfaceSelectComboBox);
    surfaceToolbar->addSeparator();
...
void SpecimenDigitiser::Plot() {
    ...
    cutMeshmapper->SetInputData(m_surfaceMaskPoly);
...
void SpecimenDigitiser::SurfaceTool() {
    if (surfaceSliderButton->isChecked()) {
        ...
        m_regPlot = nullptr;
        if (m_surfaceBlock->GetNumberOfBlocks() > 0 ||
            m_surfaceHighlightPoints->GetNumberOfPoints() > 0) {
...
void SpecimenDigitiser::PrepareSliding() {
    ...
    bool conditionOK = 1;
    if (m_fixedHighlightPoints->GetNumberOfPoints() == m_typeINOL) {
        if (m_curveNOS != 0) {
        ...
        if (m_surfacePatchUNOS == 0 && m_surfacePatchVNOS == 0 &&
            ...
            if (m_surfaceHighlightPoints->GetNumberOfPoints() == m_surfaceNOS) {
                numSurfaceSliders =
                    m_surfaceHighlightPoints->GetNumberOfPoints();
            } else {
                conditionOK = 0;
                slidingButton->setEnabled(0);
            }
    ...
    if (m_fixedHighlightPoints->GetNumberOfPoints() == m_typeINOL &&
        ...
        m_surfaceNOS == 0) {
        m_dataDigitized = 1;
        m_parent->SetTypeI(m_fixedHighlightPoints);
        landmarkButton->setEnabled(0);
        surfaceSliderButton->setEnabled(0);
        curveSliderButton->setEnabled(0);
    }
...
void SpecimenDigitiser::MakeSlide() {
    ...
    for (int i = 0; i < m_typeINOL; i++) {
        totalCoordinates.operator()(i, 0) =
            m_fixedHighlightPoints->GetPoint(i)[0];
        totalCoordinates.operator()(i, 1) =
            m_fixedHighlightPoints->GetPoint(i)[1];
        totalCoordinates.operator()(i, 2) =
            m_fixedHighlightPoints->GetPoint(i)[2];
    }
    ...
    if (m_surfaceNOS != 0 && m_surfacePatchUNOS * m_surfacePatchVNOS == 0) {
        for (int i = 0; i < m_surfaceNOS; i++) {
            totalCoordinates.operator()(i + surfaceStartIds, 0) =
                m_surfaceHighlightPoints->GetPoint(i)[0];
            totalCoordinates.operator()(i + surfaceStartIds, 1) =
                m_surfaceHighlightPoints->GetPoint(i)[1];
            totalCoordinates.operator()(i + surfaceStartIds, 2) =
                m_surfaceHighlightPoints->GetPoint(i)[2];
        }
    }
    //--------------------
    ...
    if (num >= 0 && num < m_curveCtrlBlock->GetNumberOfBlocks()) {
        ...
        if (m_surfaceCurveHighlightCtrlPoints->GetNumberOfPoints() > 2) {
            ...
            if (m_surfaceCtrlPointsPoly->GetNumberOfPoints() > 0) {
                ...
                tempMask->DeepCopy(m_surfaceMaskPoly);
    ...
    for (iterctrl->InitTraversal(); !iterctrl->IsDoneWithTraversal();
        ...
        if (counter != id) {
            ...
            if (pd->GetNumberOfPoints() > 0) {
                ...
                m_surfaceMaskPoly->DeepCopy(tempSurfaceMask);
                m_surfaceMaskPoly->Modified();
            }
        }
        counter += 1;
    }
...
void SpecimenDigitiser::FinalizeSurfaceScene() {
    ...
    if (m_surfaceBlock->GetNumberOfBlocks() == 0 &&
        m_surfaceNOS == m_surfaceHighlightPoints->GetNumberOfPoints()) {
    }
    if ((!m_surfaceBlock->GetNumberOfBlocks() == m_surfacePatchNOP) &&
        m_surfaceNOS == 0) {
        QMessageBox warning;
...
void SpecimenDigitiser::ConstructSurfaceData(vtkPolyData* CtrlPtsPoly,
    ...
    ProjectOnMesh(tempSliders, m_surfaceMaskPoly);
    delete outlineIds;
...
void SpecimenDigitiser::ConstructSurfaceData(vtkPoints* pts, vtkPoints* outputSliders,
    ...
    if (pts->GetNumberOfPoints() > 2) {
        ...
        MeshCutter(m_meshData, curveResmplPts, m_surfaceMaskPoly);
...
void SpecimenDigitiser::MakeCage(vtkPoints* pts, vtkPolyData* outPlanePoly) {
    ...
    ProjectOnMesh(planePolyDeformed, m_surfaceMaskPoly, outlineIds);
...
void SpecimenDigitiser::DoSomeTest(vtkPolyData* targetMesh, TColgp_Array2OfPnt* plane, vtkPoints* ctrlPts) {
    ...
    for (int i = 0; i < ctrlPts->GetNumberOfPoints(); i++) {
        auto tempPt = ctrlPts->GetPoint(i);
        trimmingPoints.push_back(gp_Pnt(tempPt[0], tempPt[1], tempPt[2]));
    }
    // Convert points to OCC array
    Handle(TColgp_HArray1OfPnt) pointsArray =
        new TColgp_HArray1OfPnt(1, trimmingPoints.size());
    for (int i = 0; i < trimmingPoints.size(); ++i) {
        pointsArray->SetValue(i + 1, trimmingPoints[i]);
    }
    // Interpolate a B-spline curve passing through the trimming points
    Handle(Geom_BSplineCurve) bsplineCurve;
    GeomAPI_Interpolate interpolator(pointsArray, Standard_True,
                                     1e-6);  // Tolerance can be adjusted
    interpolator.Perform();
    if (interpolator.IsDone()) {
        bsplineCurve = interpolator.Curve();
    } else {
        // Handle error: Interpolation failed
        std::cout << "Error0" << std::endl;
        return;
    }

    int numPieces = 3;
    // std::vector<TopoDS_Edge> choppedCurve;
    //  Create the filling algorithm
    BRepFill_Filling fillingAlg;

    // BRepBuilderAPI_MakeWire wireBuilder;
    //  Calculate the parameter range of the curve
    Standard_Real u1 = bsplineCurve->FirstParameter();
    Standard_Real u2 = bsplineCurve->LastParameter();

    // Calculate the parameter increment
    Standard_Real uIncrement = (u2 - u1) / numPieces;

    for (int i = 0; i < numPieces; ++i) {
        // Calculate the parameter values to split the curve
        Standard_Real uStart = u1 + i * uIncrement;
        Standard_Real uEnd = uStart + uIncrement;

        // Trim the curve to get the desired piece
        Handle(Geom_TrimmedCurve) trimmedCurve =
            new Geom_TrimmedCurve(bsplineCurve, uStart, uEnd);

        // Convert the trimmed curve to an edge
        BRepBuilderAPI_MakeEdge edgeMaker(trimmedCurve);
        TopoDS_Edge edge = edgeMaker.Edge();

        // Add the edges directly as sections to the filling algorithm
        fillingAlg.Add(edge, GeomAbs_C0, true);
        // wireBuilder.Add(edge);
        //  Store the resulting edge
        // choppedCurve.push_back(edge);
    }

    fillingAlg.Build();
    // wireBuilder.Build();

    if (fillingAlg.IsDone()) {
        // Get the resulting surface
        TopoDS_Face face = fillingAlg.Face();
        Handle(Geom_Surface) surface = BRep_Tool::Surface(face);
        Standard_Real uMin, uMax, vMin, vMax;
        int numPointsU = 10;
        int numPointsV = 10;
        surface->Bounds(uMin, uMax, vMin, vMax);
        // Calculate step sizes in U and V directions
        Standard_Real uStep = (uMax - uMin) / (numPointsU - 1);
        Standard_Real vStep = (vMax - vMin) / (numPointsV - 1);
        TopoDS_Compound compound;
        BRep_Builder builder;
        builder.MakeCompound(compound);
        // Radius of the spheres
        Standard_Real radius = 0.1;  // Adjust as needed
        for (int i = 0; i < numPointsU; ++i) {
            for (int j = 0; j < numPointsV; ++j) {
                // Calculate parameter values
                Standard_Real u = uMin + i * uStep;
                Standard_Real v = vMin + j * vStep;
                ...
                // Add the sphere to the compound
        ...
        // Write the shapes to a STEP file
        writer.Write("trimmedSpline.step");
...
void SpecimenDigitiser::CycleThroughPatches(int index) {
    m_surfaceMaskPoly->Initialize();
    vtkNew<vtkConnectivityFilter> cF;
    cF->SetInputData(m_cutMeshData);
    cF->SetExtractionModeToAllRegions();
    ...
    geometryF->Update();
    m_surfaceMaskPoly->DeepCopy(geometryF->GetOutput());
    ...
    if (landmarkButton->isChecked()) {
        ...
        if (m_iren->GetControlKey() && m_editableSurface == 1) {
            ...
            if (picker->GetPointId() != -1 &&
                ...
                // if(m_currentSurfaceId ==
                // m_surfaceCurveCtrlBlock->GetNumberOfBlocks()-1){
                /* if(m_surfaceCurveHighlightCtrlPoints->GetNumberOfPoints()==3){
                    surfaceLineEditNOP->setText(QString::fromStdString(std::to_string(m_surfacePatchNOP
                - (m_surfaceCurveCtrlBlock->GetNumberOfBlocks() +1))));
                    if(m_surfaceCurveCtrlBlock->GetNumberOfBlocks() + 1 <
                m_surfacePatchNOP){ surfaceAddButton->setEnabled(1);
                    }
    ...
    if (meshPicker->GetPointId() != -1) {
        ...
        if (landmarkButton->isChecked()) {
            ...
            if (m_surfaceNOS == 0 && !surfaceIronButton->isChecked()) {
                ...
                vtkNew<vtkCellLocator> ptLocator;
                ptLocator->SetDataSet(m_surfaceMaskPoly);
                ptLocator->BuildLocator();
        ...
        if (surfaceSliderButton->isChecked()) {
            if (m_surfaceCurveHighlightCtrlPoints->GetNumberOfPoints() > 2 &&
                ...
                if (m_surfaceCtrlPointsPoly->GetNumberOfPoints() > 0) {
                    ...
                    m_glyphSurfaceArrow->Update();
                    m_surfaceChanged = 0;
                    int numOfPatches = m_surfacePatchNOP -
...
void SpecimenDigitiser::SetSurfaceSlider(vtkPoints* pts) {
    m_surfaceHighlightPoints->Initialize();
    m_surfaceHighlightPoints->DeepCopy(pts);
    vtkNew<vtkPolyData> tempPoly;
    tempPoly->SetPoints(m_surfaceHighlightPoints);
    tempPoly->Modified();
