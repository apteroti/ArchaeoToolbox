...
void StatusReporterThread::run() {
    int i = 1;
    if(m_parentThread){
        while (! m_parentThread->isRunning() && (!m_abort)){
...
void StatusReporterThread::KillNow(){
    m_abort = true;
}

bool StatusReporterThread::Killing(){
    return m_abort;
}

StatusReporterThread::~StatusReporterThread() {}